/* Object 클래스
 - hashCode() : 인스턴스별로 고유 값을 리턴
 - equals() : 같은 인스턴스인지 비교(true/false)
 
 Map에 값을 저장할 대 키로 사용할 클래스라면,
 반드시 그 클래스는 hashCode()와 equals()를 재정의 해야한다.
 
 String + 랩퍼클래스(Byte, Short, Integer,Long, Float, Double)
 => 기본적으로 hashCode()와 equals()와 toString() 등을 재정의 하였다.
 => hashCode(); 인스턴스에 저장된 값이 같다면 같은 해시 값을 리턴한다.
 => equlas() : 인스턴스에 저장된 값이 같다면  true 리턴
...
public class MyKey {
  ...
  //password, specialNo, secreteNo 값이 같을때에는 
