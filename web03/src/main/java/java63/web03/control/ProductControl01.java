...
public class ProductControl01{  
  ...
  //   변환기 등록
  //=> 이봐 프론트 컨트롤러, 다음 메서드는 요청 파라미터를 특정 값 객체에
  //   프로퍼티 값으로 바꾸어 주는 변환기를 등록하는 매서드 야
  //    요청을 처리하기 전에 꼭호출해주라 ㅠㅠ
  //   "호출 안하면 문자열을 java.util.Date 객체로 바꿀 수 없어서,
  //    오류가 틀꺼야.꼭 호출해줘 ㅜ-ㅜ"
  @InitBinder
  public void initBinder(WebDataBinder binder) throws ParseException {
    log.debug("initbinder 호출됨");
    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    //기본은 true => 날짜가 추가되면 자동으로 계산한다. 오류없다..예외없음.
    //false로 설정=> 문자열을 엄격히 검사한다. 형식 맞지 않으면 예외발생.
    dateFormat.setLenient(false);
    //문자열을 특정타입으로 바꿀 변환기 등록
