...
public class Test11 {
  ...
  private static void doUpdate(int index) {
    ...
    Score tempScore = new Score(score.name, score.kor,
    ...
    test = scanner.nextLine();
    if(test.length() > 0)
      tempScore.kor = Integer.parseInt(test);
    
    System.out.printf("영어(%d) : ", score.eng);
    test = scanner.nextLine();
    if(test.length() > 0)
      tempScore.eng = Integer.parseInt(test);
    
    System.out.printf("수학(%d) : ", score.math);
    test = scanner.nextLine();
    if(test.length() > 0)
      tempScore.math = Integer.parseInt(test);
  ...
  private static void doDelete(int index) {
    ...
    Score score = list.get(index);
    System.out.print(score.name + "의 성적을 삭제하겠습니까? (y/n)");
    if(scanner.nextLine().equalsIgnoreCase("y")){
      list.remove(index);
