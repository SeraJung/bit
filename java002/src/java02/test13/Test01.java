/* Inner class
 ...
 => 특정 클래스 안에서만 사용하는 클래스인 경우
    그 클래스 내부에서 정의함으로 써 외부에 노출되는 것을 막을 수 있다.
 
  클래스 종류
   1. 패키지 맴버 클래스 <== 일반적으로 만드는 클래스
   2. 중첩(Inner) 클래스 
...
public class Test01 {
  ...
  static void x(){
    i = 100; //ok
    //j = 100; //error => static 매서드나 블록은 this라는 내장변수가 없다.
  }
  void y(){
    i = 100;// ok => 인스턴스 매서드나 블록은 클래스 변수를 바로 접근할 수 있다.
    j = 100;// ok  => 인스턴스 매서드나 블록은 this라는 내장변수가 있다.
  }
  //1) top level inner class
  // static 으로 선언된 inner class
  // static 이기 때문에 다른 클래스에서도 사용할 수 있다.
  // => 작은 크기의 클래스들을 묶고 싶을 때 top level 이너클래스를 사용한다.
  ...
  static class TopLevelInnerClass{
    ...
    // static 영역에서는 오로지 바깥 클래스의 static 맴버(변수, 메서드)만
    // 접근할 수 있다.
    public void tset(){
  ...
  class MemberInnerClass{
    ...
    public void test(){
      ...
      // member 이너 클래스에서는 
      //바깥 클래스의 인스턴스 멤버를 사용 할 수 있다.
      j = 300; 
      //위 코드는 컴파일 할 때 다음과 같이 바뀐다.
      //Test01.this.j = 400;
      
      // 만약 이너 클래스에 같은 이름을 가진 변수가 있다면 구분하기 위해
      // 개발자가 다음과 같이 명시적으로 선언해야 한다.
      Test01.this.j = 400;
      }
  }
