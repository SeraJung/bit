/* 크리티컬 색셕을 동기화 처리하기
 * => 여러 스래드가 크리티컬 섹션 부분을 실행하더라도
 *    문제없이 만들기
 * =>  한번에 하나의 스레드만 접근하게 만듬
 * 
 방법 : 
   1) 어떤 스레드가 크리티컬 색셕에 진입하면 잠근다.(lock)
   2) 스레드가 일을 마치고 나올 때 잠금을 해제한다.
 문법 : 
   크리티컬 색션 블록에 synchronized를 붙인다.
   예) synchronized void m(){}
   예) synchronized(객체){}
   
 뮤텍스(mutex)
 ==> 한번에 하나의 스레드 만이 크리티컬 색션에 접근하는 방식
 
 여러 스레드가 진입하더라도 계산에 영향을 끼치지 않는 코드 블록
 => 변수를 공유하지 않는다. 즉 로컬 변수만 사용한다.
 => Tread safe(스레드에 안전하다)
 => 동기화 처리를 할 필요가 없다.
 */
...
public class Test05 {
  static class Account{
    ...
    private long delay(){ //아무 의미 없는 시간 죽이기 코드.
      long l = 10l;
      double d = 3.15;
      d /= d / l ;
    ...
    // 여러 스레드가 동시에 접근했을때 문제가 발생하는 코드 블록
